...
class PlayerOne extends React.Component {
  ...
  getLazerCoords = lazerCoords => {
    ...
    const enemies = this.props.enemyAmount;
    ...
    if (!e.repeat) {
      ...
      if (this.state.jumpInterval >= max) {
        this.setState({
          jumpInterval: 0,
          startingPositionY:
            this.props.groundHeight.groundHeight.top - playerHeight
        });
        return () => cancelAnimationFrame(this.animationRef.current);
      }
  ...
  componentDidMount() {
    ...
    if (this.playerOneRef.current) {
      const playerHeight = this.playerOneRef.current.getBoundingClientRect()
        .height;
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.props.groundHeight !== prevProps.groundHeight) {
      if (this.playerOneRef.current) {
        ...
        const difference =
          this.props.groundHeight.groundHeight.top - player.height;
